...
from utils.bm25_utils import build_json_files, _coerce_issue_description


@hydra.main(version_base="1.1", config_path="../../configs/baselines", config_name="bm25_extracted_swe")
def main(cfg: BaselineConfig) -> None:
    ...
    for dp, repo_content, changed_files in data_src:
        ...
        csv_path = cfg.extracted_info_dir + "/results.csv"
        df = pd.DataFrame(pd.read_csv(csv_path))
        summarized = df['summarized_info'].iloc[row_index]
        issue_description = _coerce_issue_description(summarized)
